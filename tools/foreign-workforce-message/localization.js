(function () {
  ...
  const CUSTOM_MISSING = '[請補外語或英文內容]';
  ...
  function customValue(values, id, language, warnings) { if (language === 'zh') return (values[`${id}_zh`] || '').trim(); const foreign = (values[`${id}_foreign`] || '').trim(); if (!foreign && (values[`${id}_zh`] || '').trim()) { warnings.push({ type: 'missing_foreign_custom', field: `${id}_foreign`, message: '自訂地點、公司或文件尚未填寫外語／英文內容。' }); return CUSTOM_MISSING; } return foreign || CUSTOM_MISSING; }
  ...
  window.ForeignWorkforceMessageLocalization = { languages: LANGUAGES, languageNames: LANGUAGE_NAMES, options: OPTIONS, optionalPhrases: OPTIONAL_PHRASES, formatDateByLanguage, formatTimeByLanguage, buildValues, optionLabel, joinList, missingText: CUSTOM_MISSING };
})();
